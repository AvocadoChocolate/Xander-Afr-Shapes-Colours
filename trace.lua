...
local r = display.newRect( display.contentWidth/2, display.contentHeight/2, display.actualContentWidth, display.actualContentHeight )
    ...
    local speechtext = display.newText( "Jou beurt om die \nvorms te teken", speech.x, speech.y, speech.contentWidth/1.3,0,teachersPetFont,20 )
    speechtext:setFillColor( 0 )
		...
		homeBtn.xScale =0.40
		homeBtn.yScale =0.40
		homeBtn.x = -homeBtn.contentWidth/2
		homeBtn.y = ( yInset * 1.5 ) * 0.8
	...
	if success then 
		...
		if secondFade == true then
			...
			timer.performWithDelay( 1000, function()
						...
						if (letters[curSlide] == 'F') then
						play(letters[curSlide+1])
						elseif (letters[curSlide] == 'E') then
						play(letters[curSlide-1])
						else
